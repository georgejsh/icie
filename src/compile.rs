...
/// error.
#[evscode::config]
pub static STANDARD: evscode::Config<Standard> = Standard::Cpp17;

/// Additional compilation flags. The flags will be appended to the command line after the
/// standard, warning, debug symbols and optimization flags. These flags will be used both in Debug
/// and Release profiles.
//#[evscode::config]
//static ADDITIONAL_FLAGS: evscode::Config<String> = "";

/// Additional compilation flags used in Debug profile. The flags will be appended to the
/// command line after the standard, warning, debug symbols, optimization flags and
/// profile-independent custom flags.
#[evscode::config]
static ADDITIONAL_FLAGS_DEBUG: evscode::Config<String> = "";

/// Additional compilation flags used in Release profile. The flags will be appended to the
/// command line after the standard, warning, debug symbols, optimization flags and
/// profile-independent custom flags.
#[evscode::config]
static ADDITIONAL_FLAGS_RELEASE: evscode::Config<String> = "";

/// Additional compilation flags used in Profile profile. The flags will be appended to the
/// command line after the standard, warning, debug symbols, optimization flags and
/// profile-independent custom flags.
//#[evscode::config]
//static ADDITIONAL_FLAGS_PROFILE: evscode::Config<String> = "";
...
/// Java ; Name of the class with main function in your solution
//#[evscode::config]
//static JAVA_MAIN_CLASS_NAME: evscode::Config<String> = "Solution";
...
async fn collect_possible_sources() -> R<Vec<Path>> {
	Ok(fs::read_dir(&workspace_root()?)
		.await?
		.into_iter()
		.filter(|path| STANDARD.get().get_extenstions().iter().any(|ext| Some(*ext) == path.extension().as_deref()))
		.collect())
}
...
pub async fn compile(source: &SourceTarget, codegen: Codegen, force: bool) -> R<Executable> {
	...
	let output_path = source.with_extension(STANDARD.get().get_executable_extension());
	if !force && should_cache(&source, &output_path).await? {
		return Ok(Executable::new(output_path));
	}
	let sources = [&source];
	let standard = STANDARD.get();
...
fn get_custom_flags(codegen: Codegen) -> Vec<String> {
	let flags = format!("{}", match codegen {
		Codegen::Debug => ADDITIONAL_FLAGS_DEBUG.get(),
		Codegen::Release => ADDITIONAL_FLAGS_RELEASE.get(),
	});
...
pub async fn suggest_install_compiler() -> R<()> {
	...
	if already_checked {
		let message = "You have not compiled anything yet, should ICIE check if a compiler is installed?";
		let should_check = evscode::Message::new(message).item((), "Check", false).warning().show().await.is_some();
		if should_check {
			dummy_compiler_run().await?;
			COMPILER_INSTALL_CONFIRMED.set(&true).await;
			evscode::Message::new::<()>("Compiling was tested and it works. Good luck!").show().await;
		}
	}
	Ok(())
}
...
async fn dummy_compiler_run() -> R<()> {
	...
	let code_file = Tempfile::new("compilerinstallcheck", STANDARD.get().get_executable_extension(), code).await?;
...
pub fn executable_path(source: SourceTarget) -> R<Path> {
	Ok(source.to_path()?.with_extension(STANDARD.get().get_executable_extension()))
}
